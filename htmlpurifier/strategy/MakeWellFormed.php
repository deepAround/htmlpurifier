...
class StrategyMakeWellFormed extends \HTMLPurifier\Strategy
{
    ...
    public function execute($tokens, $config, $context)
    {
        ...
        list($zipper, $token) = Zipper::fromArray($tokens);
        if ($token === NULL) {
            return array();
        }
        ...
        for (;;
            ...
            if ($token === NULL) {
                ...
                $token = new TokenEnd($top_nesting->name);
            ...
            if ($type === 'empty' && $token instanceof TokenStart) {
                ...
                $old_token = $token;
                $token = new TokenEnd($token->name);
                $token = $this->insertBefore(
                    new TokenStart($old_token->name, $old_token->attr, $old_token->line, $old_token->col, $old_token->armor)
                );
                ...
                if (!empty($this->stack)) {
                    ...
                    if ($autoclose && $definition->info[$token->name]->wrap) {
                        ...
                        if (isset($elements[$token->name]) && isset($parent_elements[$wrapname])) {
                            $newtoken = new TokenStart($wrapname);
                    ...
                    if ($autoclose) {
                        ...
                        if ($autoclose_ok) {
                            ...
                            $new_token = new TokenEnd($parent->name);
            ...
            if (empty($this->stack)) {
                if ($escape_invalid_tags) {
                    ...
                    $token = new TokenText($generator->generateFromToken($token));
                } else {
                    if ($e) {
            ...
            for ($j = 1; $j < $c; $j++) {
                ...
                $new_token = new TokenEnd($skipped_tags[$j]->name);
...
// property of Token was to help avoid infinite loops
...
//
//  - With Injector_RemoveEmpty, we noticed that if
